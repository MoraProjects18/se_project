...
BEGIN
  SELECT start_time
     ...
     WHERE branch_id = branchId AND start_date = start_date ;
END $$
DELIMITER ;
...
BEGIN
  SELECT ticket_id,user_id,status,start_time,branch_id,email,first_name,NIC
     FROM emission_test_db.ticket natural join emission_test_db.useracc
     WHERE start_date = CURDATE() AND branch_id = branchId AND status="Open" ORDER BY start_time;
