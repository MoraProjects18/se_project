...
CREATE TABLE admin(user_id int(15) unsigned PRIMARY KEY, admin_id varchar(15), FOREIGN KEY(user_id) references useracc(user_id));
...
CREATE TABLE customer(user_id int(15) unsigned PRIMARY KEY, license_number char(8),FOREIGN KEY(user_id) REFERENCES useracc(user_id));
...
CREATE TABLE ticket(ticket_id int(15) unsigned NOT NULL AUTO_INCREMENT PRIMARY KEY , user_id int(15) unsigned, status varchar(10), start_time DateTime, end_time DateTime, FOREIGN KEY(user_id) REFERENCES customer(user_id));
